...
%
The \capsabbrev{pspace}-hardness result extends to equivalence of normal forms in \capsabbrev{mell} without units, where the weakening rule for the ``why-not'' exponential induces a similar rewiring problem.
\end{abstract}
...
\]
%
The direction $(\Rightarrow)$ is by Lemma~\ref{lem:completeness}; the direction $(\Leftarrow)$ is by Lemma~\ref{lem:soundness} and Proposition \ref{prop:retraction}. The encoding involves computing $\itn G$, $\itn\gamma$, and $\itn\delta$, then computing the parity of $\itn\gamma$ and $\itn\delta$ and adjusting $\itn\delta$ if needed; each can be done in polynomial time in the size of $(G,\gamma,\delta)$.
\end{proof}
