...
The natural basis of the encoding is the use of jumps to indicate where the weight of an edge is allocated. The main problem then becomes, how to restrict the movement of such jumps, so that they may assign weight only to the vertices of one particular edge. The central design idea of the encoding is to use the combinatorics of connecting formulae of the form $\bigtn(\bot^n)$ to formulae $\bigparr(1^m)$ to solve this problem.
...
To ensure that a weight element may be linked to only two vertices. The arithmetic of linking will be used
%
...
\vspace{20pt}
\[
\begin{array}{c@{\hspace{30pt}}c@{\hspace{30pt}}c@{\hspace{30pt}}c}
	\andexample 1 & \andexample 2 & \andexample 3 & \andexample 4 \\
...
We would like to thank Lutz Stra{\ss}burger for taking a very careful look at our construction and our proofs, and Dominic Hughes for several useful suggestions on exposition.
