...
In this section, we will give a brief introduction to multiplicative linear logic (MLL). For simplicity of exposition, we will work in the unit-only fragment of MLL. The formulae of this fragment are given by the following grammar.
%
\setMidspace{8pt}
\[
...
\begin{definition}[MLL proof equivalence]
...
\begin{itemize}
	...
	\item
	a \emph{switching graph} for $\links$ is an undirected graph over the names of $\Gamma$ with:
	\begin{itemize}
...
\section{Proof search complexity}

% TO DO:
%	n-fold bottoms & ones (below)
%   3-sat reduction for proof search

\cite{Lincoln-Mitchell-Scedrov-Shankar-1992} % NP-completeness with weakening
\cite{Kanovich-1992} % NP-completeness for MLL
\cite{Lincoln-Winkler-1994} % NP-completeness for MLL without atoms
...
%
Given a sequent $\Gamma=A_1,\dotsc,A_n$ we will write $\bigtn\Gamma$ for $A_1\!\tn\cdots\tn A_n$, and $\bigparr\Gamma$ for $A_1\!\pr\cdots\pr\! A_n$.
%
