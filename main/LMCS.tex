...
\]
%
The connectives tensor ($\tn$) and par ($\parr$) will be considered up to associativity, and \emph{duality}, indicated $\dual A$, is via DeMorgan.
%
...
The proof system we shall be working with is that consisting of just the introduction rules for the two units and the two connectives, the four rules $(\bot)$, $(\1)$, $(\parr)$, and $(\tn)$ of Figure~\ref{fig:MLL}. The problem we consider is the following.
