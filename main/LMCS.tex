...
The relation $(\toNet)$ interprets a proof $\Pi$ by a linking $\links$ as follows: $\Pi\toNet\links$ if for each $\named a\bot$ in $\Pi$, if $\Gamma$ is the context of the inference introducing $\named a\bot$, as illustrated below, then $\links(a)$ is a name in $\Gamma$.
\[
...
\begin{remark}
A main distinction with the proof nets of \cite{Blute-Cockett-Seely-Trimble-1994} is that the latter are defined as graphs whose vertices represent inferences, and whose edges represent formulae. This formulation allows an easy inclusion of cuts, which are not present in our proof nets. That the jumps from $\bot$-vertices in these graphs are attached to edges, not vertices, is a natural choice given the categorical view of $\bot$-introduction as the isomorphism $A\to A\pr\bot$.
\end{remark}
...
A refinement of proof nets introduced by Hughes \cite{Hughes-2012-nets}, is that jumps may be restricted to target only $1$-formulae. For the remainder of the paper, unless otherwise indicated, we will assume that proof nets are of this form.
...
\begin{proposition}[\cite{Blute-Cockett-Seely-Trimble-1994,Hughes-2012-nets}]
...
\]
%
The salient features of a diagram are the connectedness by $\tn$-edges and containment within a $\pr$-box; respecting these, diagrams may be laid out in the plane in any way that is convenient. More formally, then, sequents and formulae are interpreted as diagrams as follows.
%
\[
\begin{array}{c@{~}c@{~}c}
	\bot & \Rightarrow & \bullet \\[5pt]
	1	 & \Rightarrow & \circ
\end{array}
\qquad
\begin{array}{rcll}
...
To encode simple numerical properties in \capsabbrev{mll}, our primary tools will be formulae of the form $\bot\tn\dotso\tn\bot$ and $1\pr\dotso\pr1$. In particular, two formulae of the former kind with $i+1$ and $j+1$ $\bot$-occurrences respectively may form a proof net with a formula $1\pr\dotso\pr1$ if and only if the latter counts $i+j+1$ occurrences of the formula $1$. This is illustrated below left for $i=2$ and $j=3$; in the following, we will introduce the abbreviated notation used below right. 
\[
\vc{
...
\]
By $A^n$ we will denote the sequent consisting of $n$ occurrences of a formula $A$. Given a sequent $\Gamma=A_1,\dotsc,A_n$ we will write $\bigtn\Gamma$ for $A_1\!\tn\cdots\tn A_n$, and $\bigparr\Gamma$ for $A_1\!\pr\cdots\pr\! A_n$. In diagrammatic notation, a big disc labelled $n'$ will represent the formula $\bigtn(\bot^{n+1})$, and a big circle labelled $n'$ will represent the sequent $\1^{n+1}$. A formula $\bigparr(\1^{n+1})$ is represented by a disc enclosed in a box.

\[
\begin{array}{ccc}
	\1^{n+1}			&\Rightarrow& \bignodes1 \\[5pt]
	\bigparr(\1^{n+1})	&\Rightarrow& \bignodes2 \\[5pt]
	\bigtn(\bot^{n+1})  &\Rightarrow& \bignodes3 \\[5pt]
\end{array}
\qquad\qquad\qquad
\vc{
...
An instance of the 3-Partition problem is given by a multiset $I=\{i_1,\dotsc,i_{3n}\}$ of $3n$ natural number values with total sum $\sum I=nk$, for which the decision problem asks whether a partitioning exists of $I$ into $n$ triples $(i_a,i_b,i_c)$ each with sum $k$. It may be assumed that each $i\in I$ lies between $\nicefrac k4$ and $\nicefrac k2$, to relax the problem to ask whether there is a partitioning into \emph{subsets} with sum $k$. The 3-Partition problem is strongly \capsabbrev{np}-complete \cite{GareyAndJohnson}, which means it remains \capsabbrev{np}-complete with a unary encoding of the values in $I$.

A 3-Partition instance $I$ is encoded by a sequent consisting of:
\begin{itemize}
	\item a formula $\bigtn(\bot^{i+1})$ for each $i\in I$, and
	\item the formula $\bigtn(K^n)$ where $K=\bigparr(\1^{k+1})$.
\end{itemize}
The construction, including a proof net providing a solution, is illustrated below.
...
The idea behind the encoding is that in a proof net, the jumps from a formula for $i$ must connect to the same instance of $K$, since otherwise a switching graph with a cycle is created. The number of $\bot$-occurrences and $1$-occurrences in the sequent then requires that each formula $K$ is linked to by exactly three formulae. A proof net for a sequent that encodes a 3-Partition instance $I$ then directly gives a solution to $I$.
...
\begin{array}{ccccccccc}
	...
	\\ \\ &&&&&&&& \perm \\ \\
	\boat5 &\perm& \boat6 &\perm& \boat7 &\perm& \boat8 &\perm& \boat9
\end{array}
\]
\caption{The boat}
\label{fig:boat}
...
%
A \emph{constraint graph} is a graph with weighted edges and an \emph{inflow constraint}---a natural number that may be taken to be always $2$---on each vertex. A \emph{configuration} is an assignment of directions to the edges of the underlying undirected constraint graph, where at each vertex the total weight of the incoming edges is at least the inflow constraint of that vertex. A rewrite step consists of the reversal of a single edge in a configuration, such that all the inflow constraints remain satisfied.
...
This result may seem surprising at first. To give a feel for the dynamics that make this problem \capsabbrev{pspace}-hard, we shall exhibit an example where an exponentially-long reconfiguration sequence is needed.

\noindent\boat{1}\hskip3em\boat{10}

\noindent\boat{11}\hskip3em\boat{20}
...
As the linkings above illustrate, it may occur that one subformula of a weight element $A\pr B\pr C$ fills one half of a constraint element.
%
...
% TO DO: FIND A PLACE FOR:
%
% A path in a proof net or switching graph will be indicated $\path ab$, and illustrated as below.
%\[
%\begin{tikzpicture}[x=5mm,y=5mm,octo]
...
The linearity of \capsabbrev{mll} means that in a proof or proof net, there is always a certain balance to the number of $\bot$- and $\pr$-occurrences. 
%
...
For this argument we will work with $n$-ary connectives $\tn$ and $\pr$, and alternating formulae, i.e.\ every argument of a $\tn$ is a $\pr$ and vice versa.
%
The units are given by the $0$-ary connectives, and we need not rule out unary ones.
%
We will consider a given named sequent $\Gamma$, but will assume that it consists of a single formula, if necessary by introducing a $\pr$ at the root.
...
%
It is then sufficient to choose linkings so that $D\pr E\pr F$ is in a a standard linking with $X\tn Y$, with a minor adjustment: two links from $D$ to $X$ should remain exchanged, compared to the standard linking, for step 6 below.
%
