...
\begin{remark}
A main distinction with the proof nets of \cite{Blute-Cockett-Seely-Trimble-1994} is that the latter are defined as graphs whose vertices represent inferences, and whose edges represent formulae. This formulation allows an easy inclusion of cuts, which are not present in our proof nets. That the jumps from $\bot$-vertices in these graphs are attached to edges, not vertices, may seem curious at first, but it is a natural choice given the categorical view of $\bot$-introduction as the isomorphism $A\to A\pr\bot$.
\end{remark}
...
A main technical advantage of this restriction is that cut-elimination can be performed in a one-shot operation, by path composition over jumps. Without the need to define cut-elimination by local transformations, cuts may then be omitted from proof nets altogether, which justifies the present definition.
