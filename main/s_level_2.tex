...
%
Let the rightmost $1$-occurrence of each vertex-gadget $\itn v$ be named $\alt v$, and the rightmost $\bot$-occurrence of each edge-gadget $\itn e$ named $\alt e$:
\[	
	\itn v = C\parr\dotso\parr C\parr \named{\alt v}1 \qquad \itn e= W\tn\dotso\tn W\tn\named{\alt e}\bot~.
\]
To define $\itn\gamma$, firstly let $\itn\gamma(e)=\alt v$ where $\gamma(e)=v$.
%
Then successively for each edge $e$ and each weight element $W$ in $e$, if $\gamma(e)$ has a first free constraint element $C$, extend $\itn\gamma$ to include the natural linking on $W,C$; otherwise, extend $\itn\gamma$ by the natural linking on the sequent consisting of $W$ plus the first $3n+4$ free edge absorbers.
