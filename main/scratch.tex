...
\newcommand\octorollB[1]{
  \begin{tikzpicture}[x=5mm,y=-5mm,octo]
	...
	\begin{scope}[thepink,->]
		\draw (e)--(v);
	\SWITCH{#1}{1}{\draw (d)--(v);}\DEFAULT{\draw (d)--(a1a);}
		\draw[big>,bend right] (d1) to (v1); \draw[big>,bend right=20] (d2) to (v1); \draw[big>, bend right=20] (d3) to (v2);
	\SWITCH{#1}
	{3}{
...
\newcommand\octorollC{
  \begin{tikzpicture}[x=5mm,y=-5mm,octo]
	...
	\begin{scope}[thepink,->]
		...
		\draw[big>,bend left=15] (e1) to (v1);  \draw[big>,bend left=15] (e2) to (v1);  \draw[big>, bend left=15] (e3) to (v2);
	\end{scope}
  \end{tikzpicture}
}
\newcommand\octorollD{
  \begin{tikzpicture}[x=5mm,y=-5mm,octo]
	\node[big1] (a1) at (0,0) {}; \node[circ] (a1a) at (0,2) {};
	\node[big_] (d1) at (-8.5,4) {};
	\node[big_] (d2) at (-6.25,4) {};
	\draw (-4,4) node[big_] (d3a) {} -- (-3,4) node[bullet] (d3b) {};
	\draw[rounded corners] (-10,3) rectangle (-2,5);
	\draw (-2,4)--(-1,4) node[bullet] (d) {};
	\node[big_] (e1) at ( 3.5,4) {};
	\node[big_] (e2) at ( 6  ,4) {};
	...
	\begin{scope}[thepink,->]
		\draw (e)--(v); \draw (d)--(a1a);
		\draw[bend right=15] (d3b) to (v2);
		\draw[big>,bend left] (d1) to (a1); \draw[big>,bend left=20] (d2) to (a1);
		\draw[big>,bend left=20] (d3a) to (a1);
		\draw[big>,bend left=15] (e1) to (v1);  \draw[big>,bend left=15] (e2) to (v1);  \draw[big>, bend left=15] (e3) to (v2);
	\end{scope}
  \end{tikzpicture}
}
