...
%
Below left is a constraint element, below right a weight element.
\[
\begin{tikzpicture}[x=5mm,y=5mm,octo]
	\node[big1] (1) at (0,0) {$m'$}; 
	\node[big1] (2) at (3,0) {$n'$};
...
\]
To ensure that no other connections can be made, values are chosen such that $m\mod3=1$ and $n\mod3=2$, and correspondingly $i\mod3=1$, $j\mod3=0$, and $k\mod3=2$.
...
%
These will instead connect to additional $\1$-formulae, referred to as edge-absorbers, as follows.
...
For a constraint graph $G=(V,E,c,v,w)$ let $|V|$ and $|E|$ denote the number of vertices and edges, respectively, and let $|c|$ and $|w|$ denote the sum of all inflow constraints and the sum of all weights respectively:
\[
