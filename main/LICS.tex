...
%
\emph{Non-deterministic constraint graph reconfiguration} or \emph{\textsc{ncg}-reconfiguration} is the problem of deciding when two configurations for a constraint graph are connected in the graph of total configurations and reconfiguration steps.
%
\emph{Partial \textsc{ncg}-reconfiguration} is the same problem for partial configurations.
...
We will demonstrate the \textsc{pspace}-hardness of MLL proof equivalence by an encoding of partial \textsc{ncg}-reconfiguration in MLL.
...
%
These will instead connect to additional, separate $\1$-formulae, referred to as \emph{edge-absorbers}, as follows.
%
\[
\begin{tikzpicture}[x=5mm,y=5mm,octo]
	\node[big_] (A) at (0,0) {}; 
...
\]
%
In such an inappropriate linking, since both halves of the constraint element are connected, both weight elements may not be---otherwise, the linking would violate the switching condition.
%
That means the weight elements must belong to different edges; and moreover, for the same reason these two edges cannot fill another constraint element together.
%
...
The complete encoding of a constraint graph will be a sequent consisting of: 1) all vertex-gadgets combined in a single formula via tensors, 2) all edge-gadgets as individual formulae, 3) a sufficient number of edge-absorbers ($\1$-formulae).
%
...
The \emph{encoding} $\itn G$ of a constraint graph $G=(V,E,c,v,w)$ is a sequent constructed as follows.
%
...
In the above definition, the final $\1$-subformula of a vertex-gadget $\itn{v_k}$ is its \emph{indicator target}; the final $\bot$-subformula of an edge-gadget $\itn{e}$ is its \emph{indicator}; and in the completed encoding $\itn G$ the $p$ instances of $\1$ are the \emph{edge-absorbers}.
%
...
\begin{enumerate}
	\item
for a constraint element $C=C_n(i)=X\tn Y$, the \emph{standard linking} for the sequent $W,C$ links the first $\bot$ in $A$ to the first $\1$ in $X$, the first $\bot$ in $B$ and $C$ each to the first $\1$ in $Y$, and the remaining $\bot$-occurrences in $A,B,C$ to the remaining $\1$-occurrences in $X,Y$;
	\item
for $C=C_n(j)=X\tn Y$, the \emph{standard linking} for $W,C$ is defined as above, except the first $\bot$ in $B$ links to the first $\1$ in $X$;
	\item
the \emph{standard linking} for the sequent $W,\1^{3n+4}$ links the first $\bot$ in $A$, $B$, and $C$ each to the first $\1$, and the remaining $\bot$-occurrences to the remaining $\1$-occurrences.
\end{enumerate}
\end{definition}
...
%
Then successively for each weight element $W$ in $e$, if $\itn v$ has a first free constraint element $C$, extend $\itn\gamma$ to include the standard linking on $W,C$; otherwise, extend $\itn\gamma$ by the standard linking on the sequent consisting of $W$ plus the first $3n+4$ free edge absorbers.
\end{definition}
...
%
Two links from $C$ to $X$ should remain exchanged, compared to the standard linking, for step \ref{item:exchange needed} below.
%
