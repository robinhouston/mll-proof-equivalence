...
The empire $\emp v$ is determined by propagation from $v$: 1) through links; \color{red} 2) up towards subformulae; 3) into a tensor if one of its subformulae is in $\emp v-\{v\}$; 4) into a par if all its subformulae are in $\emp v-\{v\}$.%;\\ 5) except never into a par or tensor if $v$ is a subformula.
\end{proposition}
...
\color{red}
Rewiring a jump from $\named v\bot$ preserves the empire of $v$, up to that rewiring, by the first lemma below.
%
By the second lemma, the empire of any other vertex $w$ grows or shrinks, or neither, but not both.
\color{black}
