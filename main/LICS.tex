...
%
Furthermore, we will  \emph{parity}, 

%i.e.\ that of sequents of formulae constructed over $\1$, $\bot$, and $\tn$.

% TODO: mention the parity problem
...
We shall choose a variant of this formalism to suit our reduction to \capsabbrev{mll} proof equivalence.
%
A constraint graph itself will have \emph{undirected} edges, while a separate notion of \emph{configuration} imposes a direction on them.
%
A constraint graph may then be encoded as a sequent, and a configuration as a proof net.
%
As a minor adaptation, we will allow \emph{partial} configurations, where edges may be left undirected---as long as the inflow constraints are satisfied by the directed edges.
