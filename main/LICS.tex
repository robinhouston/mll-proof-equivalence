...
\begin{definition} 
A \emph{non-deterministic constraint graph} (\textsc{ncg}) $G=(V,E,c,v,w)$ consists of: a set $V$ of vertices with \emph{minimum inflow constraint} $c\colon V\to\mathbb N$; and a set $E$ of undirected edges, between vertices $v(e)=\{v_1,v_2\}\subseteq V$ and with \emph{weight} $w\colon E\to\mathbb N$.
%of at most one undirected edge $e$ per vertex-pair $v(e)=\{v_1,v_2\}$, with \emph{weight} $w\colon E\to N$.

\smallskip

A \emph{(partial) configuration} for an \textsc{ncg} is a (partial) function $\gamma\colon E\to V$ such that
\begin{itemize}
	\item
...
%
firstly, if $\gamma\perm*\delta$ for partial configurations, then these may be completed to total configurations $\gamma'\sim\delta'$ or $\gamma'=\delta'$; secondly,
%
