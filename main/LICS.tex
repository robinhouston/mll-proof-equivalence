...
%It follows that there can be no satisfactory canonical notion of proof net: if a notion of proof net were to exist where both the translation from proofs to nets and equality of proof nets are polynomial-time computable, this would imply that \capsabbrev{p}\,=\,\capsabbrev{pspace}.
...
%The translation from sequent proofs to proof nets is as follows.

%In the sequent calculus, the rule for $\bot$ introduces a $\bot$-formula into a sequent $\Gamma$, rather than an occurrence of $\1$, as in proof nets.
%
...
% 1) all vertex-gadgets combined in a single formula via tensors, 2) all edge-gadgets as individual formulae, 3) a sufficient number of weight absorbers ($\1$-formulae).
\begin{enumerate}
 	\item
...
\noindent
1.\quad The initial configuration is illustrated above. 
%
...
\noindent
4.\quad %\label{item:exchange added}
In the present configuration, the links from the weight elements form two subnets: one over the sub-sequent $A,B,X,D,1^m$, and one over the sub-sequent $C,Y,E,F,1^k$, for some $m$ and $k$.
%
...
%
It is then sufficient to choose linkings so that $D\parr E\parr F$ is in a a standard linking with $X\tn Y$, with a minor adjustment: two links from $C$ to $X$ should remain exchanged, compared to the standard linking, for step 6 %\ref{item:exchange needed}
below.
%
...
\noindent
6.\quad %\label{item:exchange needed}
The link from $B$ to $X$ is the one remaining connection between the edge-gadgets $\itn{e_i}$ and $\itn{e_j}$.
%
Lemma~\ref{lem:double exchange} allows to swap the targets of the link from $B$ to $X$ and the link from $i$, and simultaneously undo the exchange in the links from $C$ to $X$ added in step 4 %\ref{item:exchange added} 
above.
%
...
%
%Write $b(A)$ for the balance of a formula (or sequent) $A$.
%%
%Each constraint element in $\itn v$ is an instance of the formula $C=A\tn B$, where $b(A)\equiv 2$ and $b(B)\equiv 1\pmod 3$.
%%
%For $e$ inappropriate for $v$, a weight element $W=X\pr Y\pr Z$ in $\itn e$ satisfies $b(X)\equiv 2$, $b(Y)\equiv 1$, and $b(Z)\equiv 0\pmod 3$. 
%%
%To find a proof net for $W,C$ we need to assign balanced boxes to the par-formulae $A$ and $B$.
%%
%Since $e$ is inappropriate, the sequents $A,X$ and $A,X,Y$ are not balanced, while the balance $b(A,Y)\equiv 2$, $b(A,Z)\equiv 
...
%\]
%We will show that within the sequent $C,W$, it is not possible to assign a balanced box to both $\parr$-subformulae of $C$.
%%
%For each of the three $\tn$-subformulae, adding it to the box of a $\parr$ will change its balance by $3a+1$, $3(b-a)$, and $3(n-b)+2$ respectively.
%%
%In pairs, they add to the balance of a box by $3b+1$ (1st and 2nd subformula), $3(a+n-b)+3$ (1st and 3rd), and $3(n-a)+2$ (2nd and 3rd).
%%
%Since $i\neq a,b$, and since no $\tn$-subformula of $W$ can connect to more than one $\parr$-subformula of $C$ (by Proposition~\ref{prop:tensor has disjoint subnets}), it follows that $W$ can balance the box of at most one of both $\parr$-subformulae of $C$.
%
%
%Next, the vertex-gadget $\itn{v_i}$ is the formula
%\[
