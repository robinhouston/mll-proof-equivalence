...
\color{red}
if a notion of proof net were to exist where both the translation from proofs to nets and equality of proof nets are polynomial-time computable, this would imply that \capsabbrev{p}\,=\,\capsabbrev{pspace}.
\color{black}
...
\begin{proposition}[\cite{Danos-Regnier-1989}, \color{red}\cite{Lamarche-Strassburger-2006}\color{black}]
...
\color{red}
The complete encoding of a constraint graph $G$ will be a sequent $\Gamma$ consisting of:
...
%\]
%%
%The contraction of a scope corresponds to the use of a box,  may replace the switching condition as a correctness criterion:
%%
...
%	\item
%for each $\named v\parr$ the graph $\sigma(v)=(s(v),\links|_{s(v)})$ becomes a tree when all \color{red} (immediate) sub-sequents $s(w)<s(v)$ are contracted.
%\end{enumerate}
%%
%\end{proposition}
%
...
%
Firstly (Lemma~\ref{lem:rewiring within empire}), a jump from $\named v\bot$ may be rewired to exactly those $\1$-occurences that are in the empire of $v$.
%
Secondly (Lemma~\ref{lem:rewiring preserves empire}), rewiring this jump preserves the empire of $v$, up to that rewiring.
%
Thirdly (Lemma~\ref{lem:rewiring affects empires}), the empire of any other vertex $w$ will grow or shrink, or neither, but not both.
\color{black}
...
For this argument we will work with formulae over alternating $n$-ary connectives $\tn$ and $\parr$; i.e.\ every argument of a $\tn$ is a $\parr$ and vice versa.
%
The units are then given by the $0$-ary case; moreover, we need not rule out $1$-ary connectives.
%
...
%
A tensor $\bigtn(A_1,\dotsc,A_n)$ named $v$ has $n$ edges, which we shall name $v(1),\dotsc,v(n)$; a par $\named v\pr$ has one switched edge named $v(1)$; and $\named v\bot$ has the jump named $v(1)$.
%
...
%
The induced bijection associates for example the edge named $r(1)$ with the vertex $g$, the edge $g(1)$ with $h$, and the edge $h(1)$ with $a$, and it associates $f(1)$ with $f$ and $c(3)$ with $c$.
...
\]
\caption{Rewiring on a switching graph as a directed tree}
...
%
By fixing a switching for $\Gamma$, we obtain a switching graph for each of the two nets; note that the jump is named $v(1)$ in each.
%
...
%
\displayOcto{\octorollA2}\qedhere
\end{enumerate}
\end{proof}
